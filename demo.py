...
#`EGL` backend not available on Windows
...
if os.name == "posix":
    ...
    # Windows has no RLIMIT_NOFILE equivalent
    # This is the closest useful information
    open_files = process.num_handles()

    print(f"Windows detected. Open handles: {open_files}")
    print("RLIMIT_NOFILE cannot be modified on Windows.")
...
# Num of workers set to 0 to avoid issues on Windows
